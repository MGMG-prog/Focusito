...
@Composable
fun PantallaConectores2(navController: NavController,) {
    Box(
        ...
        val opciones = listOf("Aunque","Por lo tanto", "de hecho", "Por consiguiente")
        val correctAnswerIndex = 1 // "Sin embargo"

        if (correctSelected) {
            LaunchedEffect(Unit) {
                delay(1000)
                navController.navigate("pantalla_conectores_2")
            }
        }
        Column(
            modifier = Modifier
            ...
            Text(
                text = " Llegó tarde a la reunión, _________ nadie lo notó.",
                color = Color.Black,
                fontSize = 20.sp,
            ...
            opciones.forEachIndexed { index, opcion ->
                ...
                Button(
                    {navController.navigate(Screen.conector3.route)},
...
@Composable
fun hopppp() {
    PantallaConectores2(navController = rememberNavController())
}
